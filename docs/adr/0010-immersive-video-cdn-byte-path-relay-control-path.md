...
2. **The headset learns where the bytes are from a Download Descriptor**, not from the console. On every `videoDownloadStart` the headset fetches `GET /api/v1/device-videos/:videoId?deviceId=<Headset Identity>` and receives the current verified version, CDN URL and byte size (the SHA-256 this ADR originally carried was dropped by ADR 0011). The socket payload carries `videoId` only; the console never holds a byte-path URL. The endpoint trusts the Headset Identity on the same terms as the Library Mirror write (ADR 0009). Introducing a presigned URL later is a server-only change: the endpoint mints the URL and the headset already re-fetches the descriptor on a CDN 403/410.
...
- `immersiveVideo.list` for the console returns no `url` and no `checksum`; it keeps `sizeBytes` and `version` for the size warning and the "Update available" diff. (ADR 0011 later removed the checksum from the descriptor too.)
