...
function Table({ className, ...props }: React.ComponentProps<'table'>) {
  return (
    <div
      ...
      className='relative max-h-full w-full overflow-x-auto'
    >
      <table
        data-slot='table'
...
function TableHead({ className, ...props }: React.ComponentProps<'th'>) {
  return (
    <th
      ...
      className={cn(
        'h-10 px-2 text-left align-middle font-medium whitespace-nowrap text-zinc-950 dark:text-zinc-50 has-[[role=checkbox]]:pr-0 *:[[role=checkbox]]:translate-y-0.5',
        className,
      )}
      {...props}
    />
